...
function HeaderComponent() {
    return (
        <div className="pl-3 pr-3 pt-3">
            <h1 className="text-3xl pl-3 py-2 font-bold tracking-tighter sm:text-4xl md:text-4xl">Claim Item</h1>
...
function FormComponent({ onClose }) {
    ...
    const handleImageChange = (event) => {
        ...
        if (file.size > 1024 * 1024) { // size > 1MB
    ...
    return (
        <form className="grid gap-4" onSubmit={handleSubmit}>
            {message.text && <div className={`text-${message.type === 'error' ? 'red' : 'green'}-500`}>{message.text}</div>}
            <div className="grid gap-2">
                <Label htmlFor="item-description" className="text-2xl">Description of the Item</Label>
                <Textarea
            ...
            <div className="grid gap-2">
                <Label htmlFor="proof-of-ownership" className="text-2xl">Attach Image (Optional)</Label>
                <label
                ...
                >
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p className="text-base text-gray-500 dark:text-gray-400">SVG, PNG, JPG or GIF (MAX. 1MB)</p>
                    </div>
                    <Input className="hidden" id="proof-of-ownership-image" type="file" onChange={handleImageChange}/>
                </label>
                {imageError && <div className="text-red-500">{imageError}</div>}
            </div>
            <div className="grid gap-2">
                <Label htmlFor="additional-info" className="text-2xl">Additional Information</Label>
                <Textarea
            ...
            <div className="flex justify-between">
                <button className="text-2xl" type="button" onClick={onClose}>Back</button>
                <Button
