  ...
  return (
    <div className="flex justify-center mt-4">
      <button
        ...
        onClick={() => changePage(current - 1)} // Go to previous page
        disabled={current === 1} // Disable if on first page
      >
        Previous Page
      </button>
      ...
      <button
        className="h-16 text-lg mx-8 cursor-pointer overflow-visible rounded border-none bg-[#262626] px-7 text-center text-[#e5e5e5] shadow-md transition-colors duration-150 ease-out hover:bg-[#333333] active:bg-[#444444] active:text-[#ffffff]"
        onClick={() => changePage(current + 1)} // Go to next page
        disabled={current === totalPages} // Disable if on last page
      >
        Next Page
      </button>
    </div>
